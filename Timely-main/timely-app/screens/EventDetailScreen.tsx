...
export const EventDetailScreen = ({ route, navigation }) => {
  //get member detail
  const memberDetail = route.params.itemDetail;
  //get owner detail
  const ownerDetail = route.params.ownerDetail;
  ...
  const getMembers = async () => {
    //get members from current user
    if (!memberDetail.uid_owner) {
      await db
        .collection("events")
        .doc(currentUser.uid)
        .collection("list")
    ...
    // get members from your group
    if (memberDetail.uid_owner) {
      await db
        .collection("events")
        .doc(memberDetail.uid_owner)
        .collection("list")
        .doc(memberDetail.id)
        .collection("members")
        .where("status", "==", "joined")
        .onSnapshot(snapshot => {
          if (snapshot.size) {
            let members = [{ ...ownerDetail, isOwner: true }];
            snapshot.forEach(member => {
              db.collection("profiles")
                .doc(member.data().friend_id)
                .get()
